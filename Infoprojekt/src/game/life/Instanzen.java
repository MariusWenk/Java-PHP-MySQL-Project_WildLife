...
public class Instanzen {
	...
	public static int[] plättchenÜbrig = {30,30,30,30};
	...
	private static ArrayList<Tierplättchen> tierplättchen = new ArrayList<Tierplättchen>();
	...
	public static void setPlättchenBenutzt(int spieler) {
		plättchenÜbrig[spieler-1] = plättchenÜbrig[spieler-1] - 1;
	}
	
	public static void setPlättchenÜbrig(int plättchen, int i) {
		plättchenÜbrig[i] = plättchen;
	}
	
	public static int getPlättchenÜbrig(int spieler) {
		return plättchenÜbrig[spieler-1];
	}
	...
	public static void setTierplättchen(ArrayList<Tierplättchen> tierPlättchen, GameContainer Container) {
		tierplättchen = tierPlättchen;
		container = Container;
	}
	
	public static ArrayList<Tierplättchen> getTierplättchen() {
		return tierplättchen;
	}

	public static String[][] getSpeicherStringFelder() {
		Feld = new String[tierplättchen.size()][s];
		int i = 0;
		for (Tierplättchen plättchen : tierplättchen) {
			int x = plättchen.getX();
			int y = plättchen.getY();
			...
			Feld[i][2] = Integer.toString(plättchen.getTier());
			i++;
		}
		return Feld;
	}
	...
	public static String[][] getSpeicherString() {
		for(int i = 0; i<s;i++){
			...
			speicherString[1][i] = Integer.toString(plättchenÜbrig[i]);
